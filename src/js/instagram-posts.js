...
const instagramUser = require("./instagram-user.js");
...
const fetchPosts = async (id, posts = [], cursor, options = {}) => {
	...
	const variables = `after:${cursor},first:${postsLeft},id`;
	const url = `https://www.instagram.com/graphql/query?query_hash=${QUERY_HASH}&variables=${vars}`;
	const {
		data: {
			user: {
	...
	} = await fetch(`https://www.instagram.com/graphql/query?${url}`).json();
